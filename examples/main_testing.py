...
#mesh = trimesh.load_mesh('../models/teapot.STL')
...
layer_number=10
z_levels  = np.arange(*z_extents, step=z_extents[1]*(1/layer_number))
#print(z_levels)
...
#plot each section on matlabplot
for i in range(len(sections)):
    sections[i].show()
    #print(sections[i].vertices)


'''
#using for loop
for i in range(layer_number):
    sections = mesh.section(plane_origin=mesh.bounds[z_extents[1]*(1/i)], plane_normal=[0,0,1])
'''
